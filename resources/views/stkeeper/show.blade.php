        ...
        <div class="d-flex flex-column justify-content-between min-vh-100">
            ...
            <main class="flex-grow-1 py-3">
                        <div class="container-xxl">
                            <a type="button" class="btn btn-outline-secondary" aria-current="page" style="color:black; margin-bottom: 1em;" href="{{ route('stkeeper', ['table' => $table]) }}">Вернуться к назад</a>
                            <a href="{{ route('stkeeper.create', $table) }}" class="btn btn-success mb-3">Добавить запись</a>
                                    ...
                                    @foreach ($rows as $row)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a class="nav-link " style="color:black" href="{{ route('stkeeper.edit', ['table' => $table, 'id' => $row->id]) }}">Изменить</a>
                                                <form action="{{ route('stkeeper.destroy', ['table' => $table, 'id' => $row->id]) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
